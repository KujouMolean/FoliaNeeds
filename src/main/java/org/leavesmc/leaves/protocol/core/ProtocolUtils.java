...
public class ProtocolUtils {
    ...
    public static void sendEmptyPacket(ServerPlayer player, ResourceLocation id) {
       ((ConnectionHolder)(player)).getInternalConnection().send(new ClientboundCustomPayloadPacket(new DiscardedPayload(id, null)));
    }
    ...
    public static void sendBytebufPacket(@NotNull ServerPlayer player, ResourceLocation id, Consumer<? super RegistryFriendlyByteBuf> consumer) {
        ...
        consumer.accept(buf);
        ((ConnectionHolder)(player)).getInternalConnection().send(new ClientboundCustomPayloadPacket(new DiscardedPayload(id, ByteBufUtil.getBytes(buf))));
    }

    public static void sendPayloadPacket(ServerPlayer player, CustomPacketPayload payload) {
        ((ConnectionHolder)(player)).getInternalConnection().send(new ClientboundCustomPayloadPacket(payload));
    }
